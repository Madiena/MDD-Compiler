...
class WebsiteParser extends RegexParsers {
  def website: Parser[String] = """Website: \(""".r ~ page ~ """\)""".r ^^ {
    _.toString()
  }

  def page: Parser[String] = """Page: \(""".r ~ header ~ """,""" ~ body ~ """,""" ~ footer ~ """\)""".r ^^ {
    _.toString()
  }

  def header: Parser[String] = """\(Header: """.r ~ image ~ """,""".r ~ navbar ~ """\)""".r ^^ {
    _.toString()
  }

  def body: Parser[String] = """\(Body: """.r ~ repsep(bodyEl, ",") ~ """\)""".r ^^ {
    _.toString()
  }
  ...
  def footer: Parser[String] = """\(Footer:""".r ~ repsep(link, ",") ~ """\)""".r ^^ {
    _.toString()
  }

  def image: Parser[String] = """\(Image: \(""".r ~ word ~ """[.]jpg\)\)""".r ^^ {
    _.toString()
  }

  def navbar: Parser[String] = """\(Navbar: """.r ~ repsep(navbarEl, ",") ~ """\)""".r ^^ {
    _.toString()
  }

  def navbarList = """\(List: """.r ~ repsep(link, ",") ~ """\)""".r ^^ {
    _.toString()
  }
  ...
  def link = """\(Link:""".r ~ """\(""".r ~ identifier ~ """\),""".r ~ destination ~ """\)""".r ^^ {
    _.toString()
  }

  def destination: Parser[String] = """\(""".r ~ word ~ """\.html""".r ~ """\)""".r ^^ {
    _.toString()
  }

  def text: Parser[String] = """\(Text: """.r ~ headline ~ paragraph ~ """\)""".r ^^ {
    _.toString()
  }

  def headline: Parser[String] = """\(Headline:""".r ~ wrappedIdentifier ~ """\)""".r ^^ {
    _.toString()
  }

  def paragraph: Parser[String] = """\(Paragraph: """.r ~ wrappedIdentifier ~ """\)""".r ^^ {
    _.toString()
  }

  def unorderedList: Parser[String] = """\(List unordered:""".r ~ repsep(wrappedIdentifier, ",") ~ """\)""".r ^^ {
    _.toString()
  }

  def orderedList: Parser[String] = """\(List ordered:""".r ~ repsep(wrappedIdentifier, ",") ~ """\)""".r ^^ {
    _.toString()
  }

  def icon: Parser[String] = """\(Icon: \(http://www[.]""".r ~ identifier ~ """\)\)""".r ^^ {
    _.toString()
  }

  def fullTable = """\(""".r ~ tableWrapEl ~ tableRowHead ~ """,""".r ~ repsep(tableRowData, ",") ~ """\)""".r ^^ {
    _.toString()
  }

  def tableWrapEl = """Tablerow:""".r | """Table:""".r

  def tableRowHead = """\(""".r ~ tableWrapEl ~ repsep(tablehead, ",") ~ """\)""".r ^^ {
    _.toString()
  }

  def tableRowData = """\(""".r ~ tableWrapEl ~ repsep(wrappedIdentifier, ",") ~ """\)""".r ^^ {
    _.toString()
  }

  def tablehead: Parser[String] = """\(""".r ~ word ~ """\)""".r ^^ {
    _.toString()
  }

  def form = """\(Form: """.r ~ repsep(formEl, ",") ^^ {
    _.toString()
  }
  ...
  def input: Parser[String] = """\(Input: """.r ~ wrappedIdentifier ~ """\)""".r ^^ {
    _.toString()
  }

  def textArea: Parser[String] = """\(Textarea: """.r ~ wrappedIdentifier ~ """\)""".r ^^ {
    _.toString()
  }

  def wrappedIdentifier = """\(""".r ~ identifier ~ """\)""".r ^^ {
    _.toString()
  }

  def identifier: Parser[String] = """(([/\-!.:,'&a-zA-Z01-9\d\s])+)""".r ^^ {
    _.toString
  }

  def word: Parser[String] = """([_a-zA-Z]+)|(0|[1-9]\d*)""".r ^^ {
    _.toString
  }
  ...
  case class Website(page: Page) {
    override def toString = "Number of Subpages: "
  }
  ...
  case class Body(bodyElementList: List[]) {
  }

  case class BodyElement () {
...
// TODO: fix Navbar
  case class Navbar(elements: List[Link], navbarList: NavbarList) {
    val sb = new StringBuilder();
    for (link <- links) {
      sb.addString(sb.append(link), ",")
    }
  ...
  case class Input(placeholder: Placeholder) {
